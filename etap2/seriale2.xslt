...
>
				...
				<div class="wrapper">
					...
					<div class="content">
						...
						<div>
							...
							<br/>
							<h1>
								Seriale wyprodukowane w USA<br/>
							</h1>
							<xsl:apply-templates select="strona/seriale/serial[@produkcja = 'USA']"/>
							<h1>
								Seriale posiadajace wiecej lub rowne 4 sezony<br/>
							</h1>
							<xsl:apply-templates select="strona/seriale/serial[@sezonow >= 4]"/>
							<h1>
								Seriale wyprodukowane w Wlk Brytanie lub Irlandii i Kanadzie<br/>
							</h1>
							<xsl:apply-templates select="strona/seriale/serial[@produkcja='Irlandia/Kanada']
												 | strona/seriale/serial[@produkcja='Wielka Brytania']"/>
						</div>
						<div id="autor">
	...
	<xsl:template match="seriale/serial[@gatunek='Dramat']
				  | seriale/serial[@gatunek='Komedia']
				  | seriale/serial[@gatunek='Kryminał']
				  | seriale/serial[@gatunek='Dramat historyczny']
				  | seriale/serial[@gatunek='Sci-Fi']
				  | seriale/serial[@gatunek='Fantasy']
				  | seriale/serial[@gatunek='Horror']
				  " mode="kategorie">
			<div class="post">
				<span class="tytul2">
					<xsl:value-of select="daneSerialu/kluczowe/tytul"/>
	...
	<xsl:template match="serial[@produkcja = 'USA']">
		<xsl:variable name="tytul">
			<xsl:value-of select="daneSerialu/kluczowe/tytul"/>
		</xsl:variable>
	...
	<xsl:template match="serial[@sezonow >= 4]">
		<xsl:variable name="tytul">
			<xsl:value-of select="daneSerialu/kluczowe/tytul"/>
		</xsl:variable>
		<div class="post" id="{translate($tytul, ' ', '')}">
	...
	<xsl:template match="autor">
		...
		<xsl:variable name="pelnioletnosc">
			<xsl:choose>
				<xsl:when test="@wiek >= 18">
					Autor jest pełnoletni.
				</xsl:when>
				<xsl:otherwise>
